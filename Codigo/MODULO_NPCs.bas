...
'********************************************************
        ...
        If Area = MAPA_PRETORIANO Then
            ...
            For i = Areas(Area).X1 To Areas(Area).X2
                For j = Areas(Area).Y1 To Areas(Area).Y2
                    ...
                    NPCI = MapData(Npclist(NpcIndex).Pos.map).Tile(i, j).NpcIndex
                    If NPCI > 0 Then
    ...
    'Verificamos si es una criatura invocada...
    Call VerificarInvocacion(NpcIndex)
    ...
    Dim k As Long, M As Long
        ...
        For k = 1 To MAXUSERQUESTS
             ...
             With UserList(UserIndex).QuestStats.Quests(k)
    ...
    If MiNPC.MaestroUser = 0 Then
        ...
        If MiNPC.flags.WasAttackByGuards = False Then
            ...
            Call DropNPCaNPC(MiNPC, UserIndex)
...
Private Function TestSpawnTrigger(Pos As WorldPos, Optional PuedeAgua As Boolean = False) As Boolean
    ...
    If LegalPos(Pos.map, Pos.X, Pos.Y, PuedeAgua) Then
        TestSpawnTrigger = _
        MapData(Pos.map).Tile(Pos.X, Pos.Y).Trigger <> 3 And _
        MapData(Pos.map).Tile(Pos.X, Pos.Y).Trigger <> 2 And _
        MapData(Pos.map).Tile(Pos.X, Pos.Y).Trigger <> 1
    End If
...
Dim Y As Integer
    ...
    If InMapBounds(OrigPos.map, OrigPos.X, OrigPos.Y) Then
        ...
        Call CambiarOrigHeading(nIndex, MapData(map).Tile(X, Y).Trigger)
        ...
        Do While Not PosicionValida
            ...
            If LegalPosNPC(newpos.map, newpos.X, newpos.Y, PuedeAgua) And _
               Not HayPCarea(newpos) And TestSpawnTrigger(newpos, PuedeAgua) Then
                ...
                Call CambiarOrigHeading(nIndex, MapData(newpos.map).Tile(newpos.X, newpos.Y).Trigger)
                PosicionValida = True
            Else
                newpos.X = 0
                newpos.Y = 0
...
'Quitamos del mapa
MapData(Npclist(NpcIndex).Pos.map).Tile(Npclist(NpcIndex).Pos.X, Npclist(NpcIndex).Pos.Y).NpcIndex = 0
...
On Error GoTo errh
    ...
    With Npclist(NpcIndex)
        ...
        If LegalPosNPC(.Pos.map, nPos.X, nPos.Y, .flags.AguaValida = 1, .MaestroUser <> 0) Then
            ...
            'Update map and user pos
            MapData(.Pos.map).Tile(.Pos.X, .Pos.Y).NpcIndex = 0
            .Pos = nPos
...
Else
    If altpos.X <> 0 And altpos.Y <> 0 Then
        ...
        Call CambiarOrigHeading(nIndex, MapData(altpos.map).Tile(altpos.X, altpos.Y).Trigger)
...
'###################################################
    ...
    With Npclist(NpcIndex)
        ...
        .EquitandoBody = val(Leer.GetValue("NPC" & NpcNumber, "EquitandoBody"))
        ...
        Dim aux2 As String
        'quest
        aux2 = Leer.GetValue("NPC" & NpcNumber, "NumQuest")
        ...
        Else
            ...
            Npclist(NpcIndex).NumQuest = val(aux2)

            ReDim Npclist(NpcIndex).QuestNumber(1 To Npclist(NpcIndex).NumQuest) As Byte

            For LoopC = 1 To Npclist(NpcIndex).NumQuest
                Npclist(NpcIndex).QuestNumber(LoopC) = val(Leer.GetValue("NPC" & NpcNumber, "QuestNumber" & LoopC))
            Next LoopC

        End If
        'Nuevo sistema de Drop Quest

        aux2 = Leer.GetValue("NPC" & NpcNumber, "NumDropQuest")
        ...
        Else
            ...
            Npclist(NpcIndex).NumDropQuest = val(aux2)

            ReDim Npclist(NpcIndex).DropQuest(1 To Npclist(NpcIndex).NumDropQuest) As String

            For LoopC = 1 To Npclist(NpcIndex).NumDropQuest
                Npclist(NpcIndex).DropQuest(LoopC) = Leer.GetValue("NPC" & NpcNumber, "DropQuest" & LoopC)
            Next LoopC
       ...
       'Nuevo sistema de Drop NPC
        ...
        If LenB(aux2) = 0 Then
            Npclist(NpcIndex).NumDropNPC = 0

        Else

            Npclist(NpcIndex).NumDropNPC = val(aux2)

            ReDim Npclist(NpcIndex).DropNPC(1 To Npclist(NpcIndex).NumDropNPC) As String

            For LoopC = 1 To Npclist(NpcIndex).NumDropNPC
                Npclist(NpcIndex).DropNPC(LoopC) = Leer.GetValue("NPC" & NpcNumber, "DropNPC" & LoopC)
            Next LoopC
        ...
        With .flags
            ...
            .VuelveOrigPos = val(Leer.GetValue("NPC" & NpcNumber, "VuelveOrigPos"))
            ...
            .Colorsangre = val(Leer.GetValue("NPC" & NpcNumber, "Colorsangre"))
        End With
