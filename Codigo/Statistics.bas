...
Public Sub UserConnected(ByVal UserIndex As Integer)
    ...
    trainningInfo(UserIndex).trainningTime = GetByCampo("SELECT TrainningTime FROM pjs WHERE Nombre=" & Comillas(UserList(UserIndex).Name), "TrainningTime")
...
Public Sub UserDisconnected(ByVal UserIndex As Integer)
    With trainningInfo(UserIndex)
        ...
        Execute ("UPDATE pjs SET TrainningTime=" & CStr(.trainningTime) & " WHERE Nombre=" & Comillas(UserList(UserIndex).Name))
    End With
End Sub
