...
Public Const MAXUSERQUESTS As Integer = 7  'Máxima cantidad de quests que puede tener un usuario al mismo tiempo.
...
Dim tmpStr As String
    ...
    For i = 1 To MAXUSERQUESTS
        With UserList(UserIndex).QuestStats.Quests(i)
            ...
            If .QuestIndex Then
                ...
                End If
            End If
           Call Execute("UPDATE pjs SET Q" & i & "=" & Comillas(tmpStr) & " WHERE Id=" & UserList(UserIndex).MySQLId)
    ...
    With UserList(UserIndex).QuestStats
        ...
        If .NumQuestsDone Then
            ...
            Next i
        End If
        Call Execute("UPDATE pjs SET QuestsDone=" & Comillas(tmpStr) & " WHERE Id=" & UserList(UserIndex).MySQLId)
