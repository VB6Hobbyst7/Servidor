    ...
    With UserList(UserIndex)
        ...
        If res <= Suerte Then
            ...
            If .flags.Navegando = 0 Then
                Call UsUaRiOs.SetOculto(UserIndex, .Char.CharIndex, True)
                'Call WriteConsoleMsg(UserIndex, "¡Te has escondido entre las sombras!", FontTypeNames.FONTTYPE_INFO)
                Call WriteMultiMessage(UserIndex, eMessages.TeHasEscondidoEntreLasSombras)
        ...
        Else
            ...
            If Not .flags.UltimoMensaje = 4 Then
                'Call WriteConsoleMsg(UserIndex, "¡No has logrado esconderte!", FontTypeNames.FONTTYPE_INFO)
                 Call WriteMultiMessage(UserIndex, eMessages.NoHasLogradoEsconderte)
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If .Invent.BarcoObjIndex = 0 Then
            If TieneObjetos(476, 1, UserIndex) Then
                .Invent.BarcoObjIndex = 476
            ElseIf TieneObjetos(475, 1, UserIndex) Then
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If .flags.Equitando = True Then
            'Call WriteConsoleMsg(UserIndex, "Debes dejar de montar para usar una embarcación.", FontTypeNames.FONTTYPE_INFO)
            Call WriteMultiMessage(UserIndex, eMessages.mUserEquitando)
            Exit Sub
        End If
        
        If .flags.Embarcado > 0 Then
            'Call WriteConsoleMsg(UserIndex, "No puedes usar este objeto aquí", FontTypeNames.FONTTYPE_INFO)
            Call WriteMultiMessage(UserIndex, eMessages.UserNoPuedeUsarObjetoAqui)
            Exit Sub
        End If
        ...
        If .Stats.UserSkills(eSkill.Navegacion) / ModNave < Barco.MinSkill Then
            ...
            Call WriteMultiMessage(UserIndex, eMessages.UserNoTieneConocimientosNecesarios)
        ...
        If .flags.Nadando = True Then
                'Call WriteConsoleMsg(UserIndex, "Tienes tu embarcación anclada, vé a buscarla!", FontTypeNames.FONTTYPE_INFO)
                Call WriteMultiMessage(UserIndex, eMessages.UserTieneEmbarcacionAnclada)
            Exit Sub
        End If
        ...
        If .flags.Navegando = 0 Then
            ...
            If .flags.Muerto = 0 Then
                ...
                If .flags.Oculto = 1 Then
                    ...
                    Call UsUaRiOs.SetInvisible(UserIndex, .Char.CharIndex, False)
                    'Call WriteConsoleMsg(UserIndex, "¡Has vuelto a ser visible!", FontTypeNames.FONTTYPE_INFO)
                    Call WriteMultiMessage(UserIndex, eMessages.UserSeVuelveVisible)
                End If
            ...
            If .flags.Muerto = 0 Then
                ...
                If .clase = eClass.Pirat Then
                    If .flags.Oculto = 1 Then
                        ...
                        'Call WriteConsoleMsg(UserIndex, "¡Has recuperado tu apariencia normal!", FontTypeNames.FONTTYPE_INFO)
                        Call WriteMultiMessage(UserIndex, eMessages.UserRecuperaSuAparienciaNormal)
                    End If
                End If
                ...
                If .Invent.WeaponEqpObjIndex > 0 Then
                    .Char.WeaponAnim = ObjData(.Invent.WeaponEqpObjIndex).WeaponAnim
                     If .Invent.WeaponEqpObjIndex = BALA_CANON Then
   ...
   With UserList(VictimIndex)
        ...
        If .Invent.EscudoEqpObjIndex > 0 Then
            ...
            If Resultado <= Probabilidad Then
                ...
                Call WriteMultiMessage(UserIndex, eMessages.HasDesequipadoEsudoOponente)
    ...
    If PuedeConstruirHerreria(ItemIndex) Then
        ...
        If .clase = eClass.Worker Then
            ...
            Else
                ...
                Call WriteMultiMessage(UserIndex, eMessages.NoTienesSuficienteEnergia)
                Exit Sub
            End If
        Else
            'Chequeamos que tenga los puntos antes de sacarselos
        ...
        Select Case ObjData(ItemIndex).OBJType
            ...
            Case eOBJType.otWeapon
                ...
                Call WriteTooltip(UserIndex, .Pos.X, .Pos.Y, 5, "+" & CantidadItems & strObj)
...
If Npclist(NpcIndex).MaestroUser = UserIndex Then
    ...
    Call WriteMultiMessage(UserIndex, eMessages.YaDomasteALaCriatura)
    Exit Sub
End If
...
If Npclist(NpcIndex).MaestroNpc > 0 Or Npclist(NpcIndex).MaestroUser > 0 Then
    ...
    Call WriteMultiMessage(UserIndex, eMessages.LaCriaturaTieneAmo)
    Exit Sub
End If
...
If Not PuedeDomarMascota(UserIndex, NpcIndex) Then
    ...
    Call WriteMultiMessage(UserIndex, eMessages.NoPodesDomarDosCriaturasIguales)
    Exit Sub
End If
...
If Npclist(NpcIndex).EquitandoBody > 0 Then
    Call DomarMascotaMontura(UserIndex, NpcIndex, puntosDomar, puntosRequeridos)
    Exit Sub
    
'Si no...
ElseIf UserList(UserIndex).NroMascotas < MAXMASCOTAS Then
    ...
    If puntosRequeridos <= puntosDomar And RandomNumber(1, 5) = 1 Then
        ...
        If Not CanStay Then
            ...
            Call WriteMultiMessage(UserIndex, eMessages.NoMascotasZonaSegura)
        End If

    Else
        If Not UserList(UserIndex).flags.UltimoMensaje = 5 Then
            'Call WriteConsoleMsg(UserIndex, "No has logrado domarlo!.", FontTypeNames.FONTTYPE_INFO)
...
Else
    ...
    Call WriteMultiMessage(UserIndex, eMessages.NoPodesControlarMasCriaturas)
End If
...
'***************************************************
    ...
    If Npclist(NpcIndex).EquitandoBody > 0 Then
        If UserList(UserIndex).flags.NpcMonturaIndex = 0 Then PuedeDomarMascota = True
        Exit Function
    Else
...
Sub DoAdminInvisible(ByVal UserIndex As Integer)
    ...
    If UserList(UserIndex).flags.AdminInvisible = 0 Then
        ...
        If UserList(UserIndex).flags.Mimetizado = 1 Then
            ...
            UserList(UserIndex).Char.alaIndex = UserList(UserIndex).CharMimetizado.alaIndex
        ...
        UserList(UserIndex).flags.OldAlas = UserList(UserIndex).Char.alaIndex
    ...
    Else
        ...
        UserList(UserIndex).Char.alaIndex = UserList(UserIndex).flags.OldAlas
    End If
...
If MapData(map).Tile(X, Y).ObjInfo.ObjIndex <> 58 Then
...
If MapData(map).Tile(X, Y).ObjInfo.Amount < 3 Then
...
If exito = 1 Then
    ...
    Obj.Amount = MapData(map).Tile(X, Y).ObjInfo.Amount \ 3
...
If res <= DificultadPescar Then
    ...
    CantidadItems = CantidadItems * MultiplicadorOficio
        
    With MiObj
        .Amount = CantidadItems
        .ObjIndex = PECES_POSIBLES.PESCADO1
    End With
    ...
    If MiObj.Amount = 1 Then
        Call WriteMultiMessage(UserIndex, eMessages.HasPescado)
    Else
        Call WriteMultiMessage(UserIndex, eMessages.HasPescadoAlgunosPeces)
    End If
    
    Call SubirSkill(UserIndex, Pesca)
    
    Call WriteTooltip(UserIndex, .Pos.X, .Pos.Y, 2, "+" & MiObj.Amount)
...
If .flags.Navegando = 0 Then
    ...
    Call WriteMultiMessage(UserIndex, eMessages.LaRedSeUsaEnMarAbierto)
    Exit Sub
End If
...
If Suerte > 0 Then
    ...
    If res <= DificultadPescar Then
        ...
        Call WriteTooltip(UserIndex, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y, 2, "+" & MiObj.Amount)
    ...
    With UserList(LadrOnIndex)
        ...
        If .flags.Seguro Then
            If Not Criminal(VictimaIndex) Then
                ...
                Call WriteMultiMessage(LadrOnIndex, eMessages.DebesQuitarSeguroParaRobarCiudadano)
                Exit Sub
            End If
        Else
            If .Faccion.ArmadaReal = 1 Then
                If Not Criminal(VictimaIndex) Then
                    ...
                    Call WriteMultiMessage(LadrOnIndex, eMessages.RealNoRobaCiudadanos)
                    Exit Sub
                End If
            End If
        End If
        ...
        If UserList(VictimaIndex).Faccion.FuerzasCaos = 1 And .Faccion.FuerzasCaos = 1 Then
            ...
            Call WriteMultiMessage(LadrOnIndex, eMessages.CaosNoRobaCaos)
            Exit Sub
        End If
        ...
        If .Stats.MinSta < 15 Then
            If .genero = eGenero.Hombre Then
                ...
                Call WriteMultiMessage(LadrOnIndex, eMessages.EstasMuyCansadoParaRobar)
            Else
                'Call WriteConsoleMsg(LadrOnIndex, "Estás muy cansada para robar.", FontTypeNames.FONTTYPE_INFO)
                Call WriteMultiMessage(LadrOnIndex, eMessages.EstasMuyCansadaParaRobar)
            End If
        ...
        If UserList(VictimaIndex).flags.Privilegios And PlayerType.User Then
            ...
            If res < 3 Then 'Exito robo
                ...
                If (RandomNumber(1, 50) < 25) And (.clase = eClass.Thief) Then
                    ...
                    Else
                        ...
                        Call WriteMultiMessage(LadrOnIndex, eMessages.UserNoTieneObjetos, , , , .Name)
                    End If
                Else 'Roba oro
                    If UserList(VictimaIndex).Stats.GLD > 0 Then
                        ...
                        Call WriteMultiMessage(LadrOnIndex, eMessages.LeHasRobadoMonedasDeOro, N, , , UserList(VictimaIndex).Name)
                    ...
                    Else
                        ...
                        Call WriteMultiMessage(LadrOnIndex, eMessages.NoTieneOro, , , , UserList(VictimaIndex).Name)
                    End If
                End If
            ...
            Else
                ...
                Call WriteMultiMessage(LadrOnIndex, eMessages.NoHasLogradoRobarNada)
                'Call WriteConsoleMsg(VictimaIndex, "¡" & .Name & " ha intentado robarte!", FontTypeNames.FONTTYPE_INFO)
                Call WriteMultiMessage(LadrOnIndex, eMessages.NoHasLogradoRobarNada, , , , .Name)
                Call FlushBuffer(VictimaIndex)
...
If flag Then
    ...
    If UserList(LadrOnIndex).clase = eClass.Thief Then
        ...
        Call WriteMultiMessage(LadrOnIndex, eMessages.HasRobadoObjetos, MiObj.Amount, , , ObjData(MiObj.ObjIndex).Name)
    Else
        'Call WriteConsoleMsg(LadrOnIndex, "Has hurtado " & MiObj.Amount & " " & ObjData(MiObj.ObjIndex).Name, FontTypeNames.FONTTYPE_INFO)
        Call WriteMultiMessage(LadrOnIndex, eMessages.HasHurtadoObjetos, MiObj.Amount, , , ObjData(MiObj.ObjIndex).Name)
    End If
Else
...
'***************************************************
    ...
    If RandomNumber(1, 100) <= Suerte Then
        ...
        Else
            ...
            Call WriteMultiMessage(UserIndex, eMessages.HasGolpeadoCriticamenteCriatura, daño)
...
If res <= DificultadTalar Then
    ...
    With MiObj
        .Amount = CantidadItems
        .ObjIndex = IIf(DarMaderaElfica, LeñaElfica, Leña)
    End With
    ...
    Call WriteMultiMessage(UserIndex, eMessages.HasConseguidoLena)
    
    Call WriteTooltip(UserIndex, .Pos.X, .Pos.Y, 4, "+" & MiObj.Amount)
...
If res <= DificultadMinar Then
    ...
    Call WriteTooltip(UserIndex, .Pos.X, .Pos.Y, 3, "+" & MiObj.Amount)
    
    Call WriteMultiMessage(UserIndex, eMessages.HasExtraidoMinerales)
...
If UserList(UserIndex).Stats.MinMAN >= UserList(UserIndex).Stats.MaxMAN Then
    ...
    Call WriteMultiMessage(UserIndex, eMessages.TerminasDeMeditar)
...
If res = 1 Then
    ...
    If Not UserList(UserIndex).flags.UltimoMensaje = 22 Then
        ...
        Call WriteMultiMessage(UserIndex, eMessages.HasRecuperadoMana, Cant)
...
If (res < 20) Then
    ...
    Else
        ...
        Call WriteMultiMessage(UserIndex, eMessages.UserNoTieneObjetos, , , , UserList(VictimaIndex).Name)
    End If
End If
End Sub
