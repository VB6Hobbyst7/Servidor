...
On Error GoTo errorh
    ...
    If TeleFrag > 0 Then
        ...
        If (LegalPos(wp2.map, wp2.X, wp2.Y)) Then
            ...
            MapData(wp2.map).Tile(wp2.X, wp2.Y).NpcIndex = TeleFrag
        Else
            ''TELEFRAG!!!
...
On Error GoTo errorh
    ...
    For X = NPCPosX + (azar * 8) To NPCPosX + (azar * -8) Step -azar
        For Y = NPCPosY + (azar2 * 7) To NPCPosY + (azar2 * -7) Step -azar2
            NPCAlInd = MapData(NPCPosM).Tile(X, Y).NpcIndex   ''por si implementamos algo contra NPCs
            PJEnInd = MapData(NPCPosM).Tile(X, Y).UserIndex
...
If Npclist(npcind).flags.Paralizado = 1 Or Npclist(npcind).flags.Inmovilizado = 1 Then Exit Sub

If Not NPCPosM = 2 Then Exit Sub
...
If (BestTarget > 0) Then
    ...
    If NPCPosX < Zonas(MAPA_PRETORIANO).X1 + 31 Then
        
        Call GreedyWalkTo(npcind, 2, ALCOBA1_X + ((BTx - ALCOBA1_X) \ 2), ALCOBA1_Y + ((BTy - ALCOBA1_Y) \ 2))
        'GreedyWalkTo npcind, MAPA_PRETORIANO, ALCOBA1_X + ((BTx - ALCOBA1_X) \ 2), ALCOBA1_Y + ((BTy - ALCOBA1_Y) \ 2)
    Else
        Call GreedyWalkTo(npcind, 2, ALCOBA2_X + ((BTx - ALCOBA2_X) \ 2), ALCOBA2_Y + ((BTy - ALCOBA2_Y) \ 2))
...
Else
    ...
    If NPCPosX < Zonas(MAPA_PRETORIANO).X1 + 31 Then Xc = ALCOBA1_X Else Xc = ALCOBA2_X
    Yc = ALCOBA1_Y
    ...
    For X = Xc - 16 To Xc + 16
        For Y = Yc - 14 To Yc + 14
            If Not (X <= NPCPosX + 8 And X >= NPCPosX - 8 And Y >= NPCPosY - 7 And Y <= NPCPosY + 7) Then
                ...
                PJEnInd = MapData(NPCPosM).Tile(X, Y).UserIndex     ''por si implementamos algo contra NPCs
                If (PJEnInd > 0) Then
                    If Not (UserList(PJEnInd).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1 Or UserList(PJEnInd).flags.Muerto = 1) Then
                        ...
                        Call GreedyWalkTo(npcind, 2, UserList(PJEnInd).Pos.X, UserList(PJEnInd).Pos.Y)
                        Exit Sub
                    End If
                End If
            End If
...
'        azar = 1
    ...
    If Not (quehacer = 3) Then      ''si no ta matandose
        ...
        If (BestTarget = 0) And (Npclist(npcind).CanAttack = 1) Then
            Call VolverAlCentro(npcind)
            Exit Sub
        End If
        
        PJEnInd = MapData(NPCPosM).Tile(NPCPosX - 1, NPCPosY).UserIndex
        If (PJEnInd > 0) Then
        ...
        PJEnInd = MapData(NPCPosM).Tile(NPCPosX, NPCPosY - 1).UserIndex
        If PJEnInd > 0 Then
            If Not (UserList(PJEnInd).flags.Muerto = 1) And Not (UserList(PJEnInd).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1) Then
                Call VolverAlCentro(npcind)
                Exit Sub
...
On Error GoTo errorh
    ...
    For X = NPCPosX - 8 To NPCPosX + 8
        For Y = NPCPosY - 7 To NPCPosY + 7
            'scan combat field
            NPCAlInd = MapData(NPCPosM).Tile(X, Y).NpcIndex
            PJEnInd = MapData(NPCPosM).Tile(X, Y).UserIndex
            If (Npclist(npcind).CanAttack = 1) Then   ''saltea el analisis si no puede atacar para evitar cuentas
                If (NPCAlInd > 0) Then
                    ...
                    If ((Npclist(NPCAlInd).flags.Paralizado = 1 Or Npclist(NPCAlInd).flags.Inmovilizado = 1) And e_p > 0 And e_p < 6) Then
        ...
        If BestTarget > 0 Then
            ...
            For headingloop = eHeading.NORTH To eHeading.WEST
                ...
                If InMapBounds(nPos.map, nPos.X, nPos.Y) Then
                    UI = MapData(nPos.map).Tile(nPos.X, nPos.Y).UserIndex
                    If UI > 0 Then
...
On Error GoTo errorh
            ...
            If (Npclist(npcind).CanAttack = 1) Then   ''saltea el analisis si no puede atacar para evitar cuentas
                If (NPCAlInd > 0) Then  ''allie?
                    If (esPretoriano(NPCAlInd) = 0) Then
                        If (Npclist(NPCAlInd).MaestroUser > 0) And (Not (Npclist(NPCAlInd).flags.Paralizado > 0)) And Npclist(NPCAlInd).flags.Inmovilizado = 0 Then
                ...
                ElseIf (PJEnInd > 0) Then ''aggressor
                    If Not (UserList(PJEnInd).flags.Muerto = 1) And UserList(PJEnInd).flags.AdminPerseguible Then
                        If (UserList(PJEnInd).flags.Inmovilizado = 0) Then
    ...
    Case 0
        ...
        If NPCPosX < Zonas(MAPA_PRETORIANO).X1 + 31 Then centroX = ALCOBA1_X Else centroX = ALCOBA2_X
        centroY = ALCOBA1_Y
        ...
        For X = centroX - 16 To centroX + 16
            For Y = centroY - 15 To centroY + 15
                ...
                ''si no es un tile ya analizado... (evito cuentas)
                    NPCAlInd = MapData(NPCPosM).Tile(X, Y).NpcIndex
                    If NPCAlInd > 0 Then
                        If (esPretoriano(NPCAlInd) > 0 And (Npclist(NPCAlInd).flags.Paralizado = 1 Or Npclist(NPCAlInd).flags.Inmovilizado = 1)) Then
...
'                            GreedyWalkTo npcind, NPCPosM, Npclist(NPCAlInd).Pos.X, Npclist(NPCAlInd).Pos.Y
    ...
    Case 3  '' ataque a mascotas
        If Not (Npclist(BestTarget).flags.Paralizado = 1) Or Npclist(BestTarget).flags.Inmovilizado = 0 Then
...
On Error GoTo errorh
    ...
    Select Case bs
        ...
        Case 0
            ...
            For X = Zonas(MAPA_PRETORIANO).X1 To Zonas(MAPA_PRETORIANO).X2
                For Y = Zonas(MAPA_PRETORIANO).Y1 To Zonas(MAPA_PRETORIANO).Y2
                    PJInd = MapData(NPCPosM).Tile(X, Y).UserIndex
                    MascotaInd = MapData(NPCPosM).Tile(X, Y).NpcIndex
                    If PJInd > 0 Then
        ...
        Else    ''x completitud, esta en la misma Y
            ...
            Else
                ...
                If Npclist(npcorig).CanAttack = 1 And (RandomNumber(1, 100) > 95) Then
                    ...
                    Npclist(npcorig).AttackTimer = TIMER_ATTACK
                End If
            End If
        End If
...
On Error GoTo errorh
    ...
    If NpcMap = 2 Then
        ''35,25 y 67,25 son las posiciones del rey
        If NPCPosX < Zonas(MAPA_PRETORIANO).X1 + 39 Then    ''esta a la izquierda
...
''de los clerigos
    ...
    retvalue = Npclist(npcind).Pos.Y > ALCOBA1_Y + 14
    
    If Not Npclist(npcind).Pos.map = 2 Then
        EstoyMuyLejos = False
    Else
...
On Error GoTo errorh
    ...
    If Npclist(npcind).Pos.X < 50 Then
        retvalue = Npclist(npcind).Pos.X < ALCOBA1_X + 8 And Npclist(npcind).Pos.X > ALCOBA1_X - 8
    Else
        retvalue = Npclist(npcind).Pos.X < ALCOBA2_X + 8 And Npclist(npcind).Pos.X > ALCOBA2_X - 8
    End If
    
    retvalue = retvalue And Npclist(npcind).Pos.Y > ALCOBA1_Y - 8 And Npclist(npcind).Pos.Y < ALCOBA1_Y + 8
...
''on error resume next
    ...
    If (Npclist(npcind).Pos.map = 2) And (UserList(PJEnInd).Pos.map = 2) Then
        ...
        '' 8+8+8+8 x 7+7+7+7
        retvalue = PJPosX > Zonas(MAPA_PRETORIANO).X1 And PJPosX < Zonas(MAPA_PRETORIANO).X1 + 31 And NPCPosX <= Zonas(MAPA_PRETORIANO).X1 + 33 'And NPCPosX < 49
        retvalue = retvalue And (PJPosY > Zonas(MAPA_PRETORIANO).Y1 And PJPosY < Zonas(MAPA_PRETORIANO).Y2) 'And NPCPosY > 14 And NPCPosY < 50)
        retValue2 = PJPosX > Zonas(MAPA_PRETORIANO).X1 + 34 And PJPosX < Zonas(MAPA_PRETORIANO).X2 And NPCPosX > Zonas(MAPA_PRETORIANO).X1 + 34 'And NPCPosX < 81
        retValue2 = retValue2 And (PJPosY > Zonas(MAPA_PRETORIANO).Y1 And PJPosY < Zonas(MAPA_PRETORIANO).Y2) 'And NPCPosY > 14 And NPCPosY < 50)
...
On Error GoTo errorh
    ...
    If retvalue Then
        ...
        Pj = MapData(NPCPosM).Tile(NPCPosX + 1, NPCPosY).UserIndex
        If Pj > 0 Then
        ...
        Pj = MapData(NPCPosM).Tile(NPCPosX, NPCPosY + 1).UserIndex
        If Pj > 0 Then
            retvalue = retvalue Or UserList(Pj).flags.Muerto = 1
        End If
...
On Error GoTo errorh
    ...
    If LegalPos(NPCPosM, NPCPosX + 1, NPCPosY + 1) Then
        ...
        'Update map and npc pos
        MapData(NPCPosM).Tile(NPCPosX, NPCPosY).NpcIndex = 0
        Npclist(npcind).Pos.Y = NPCPosY + 1
        Npclist(npcind).Pos.X = NPCPosX + 1
        Npclist(npcind).Char.heading = eHeading.SOUTH
        MapData(NPCPosM).Tile(NPCPosX + 1, NPCPosY + 1).NpcIndex = npcind
