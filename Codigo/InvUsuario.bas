            ...
            For j = UserList(UserIndex).Pos.X - 10 To UserList(UserIndex).Pos.X + 10
                For k = UserList(UserIndex).Pos.Y - 10 To UserList(UserIndex).Pos.Y + 10
                    If InMapBounds(M, j, k) Then
                        If MapData(M).Tile(j, k).UserIndex > 0 Then
                            Cercanos = Cercanos & UserList(MapData(M).Tile(j, k).UserIndex).Name & ","
                        End If
                    End If
                Next k
            Next j
...
If Num > 0 Then
  ...
  If MapData(UserList(UserIndex).Pos.map).Tile(X, Y).ObjInfo.ObjIndex = 0 Or MapData(UserList(UserIndex).Pos.map).Tile(X, Y).ObjInfo.ObjIndex = UserList(UserIndex).Invent.Object(Slot).ObjIndex Then
        ...
        If Num + MapData(UserList(UserIndex).Pos.map).Tile(X, Y).ObjInfo.Amount > MAX_INVENTORY_OBJS Then
            Num = MAX_INVENTORY_OBJS - MapData(UserList(UserIndex).Pos.map).Tile(X, Y).ObjInfo.Amount
        End If
...
MapData(map).Tile(X, Y).ObjInfo.Amount = MapData(map).Tile(X, Y).ObjInfo.Amount - Num

If MapData(map).Tile(X, Y).ObjInfo.Amount <= 0 Then
    MapData(map).Tile(X, Y).ObjInfo.ObjIndex = 0
    MapData(map).Tile(X, Y).ObjInfo.Amount = 0
...
If Obj.ObjIndex > 0 And Obj.ObjIndex <= UBound(ObjData) Then
    ...
    If MapData(map).Tile(X, Y).ObjInfo.ObjIndex = Obj.ObjIndex Then
        MapData(map).Tile(X, Y).ObjInfo.Amount = MapData(map).Tile(X, Y).ObjInfo.Amount + Obj.Amount
    Else
        MapData(map).Tile(X, Y).ObjInfo = Obj
...
'¿Hay algun obj?
If MapData(UserList(UserIndex).Pos.map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).ObjInfo.ObjIndex > 0 Then
    '¿Esta permitido agarrar este obj?
    If ObjData(MapData(UserList(UserIndex).Pos.map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).ObjInfo.ObjIndex).Agarrable <> 1 Then
    
        If HayAgua(UserList(UserIndex).Pos.map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y) = True And UserList(UserIndex).flags.Nadando = False Then
            Call WriteConsoleMsg(UserIndex, "Debes tirarte al agua para agarrar los objetos que se encuentran en el Mar.", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        ...
        Obj = ObjData(MapData(UserList(UserIndex).Pos.map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).ObjInfo.ObjIndex)
        MiObj.Amount = MapData(UserList(UserIndex).Pos.map).Tile(X, Y).ObjInfo.Amount
        MiObj.ObjIndex = MapData(UserList(UserIndex).Pos.map).Tile(X, Y).ObjInfo.ObjIndex
...
Public Sub DesequiparTodosLosItems(ByVal UserIndex As Integer)
...
Select Case Obj.OBJType
    ...
    Case eOBJType.otPociones
        ...
        Select Case .flags.TipoPocion
            ...
            Case 7  ' Pocion Chiquitolin
                If .flags.Privilegios Then
                
                    If .Counters.Saliendo = True Then
                        Call WriteConsoleMsg(UserIndex, "No puedes usar este objeto ahora....", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                
                    If .flags.Equitando = True Then
                        Call WriteConsoleMsg(UserIndex, "No puedes usar este objeto equitando....", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                    
                    If .flags.Navegando = 1 Or .flags.Nadando = True Or .flags.Embarcado > 0 Then
                        Call WriteConsoleMsg(UserIndex, "No puedes usar este objeto en el mar....", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                    
                    If .flags.Chiquito = False Then
                        Call QuitarUserInvItem(UserIndex, Slot, 1)
                        Call Chiquitolin(UserIndex, True)
                        Call WriteConsoleMsg(UserIndex, "Sientes un gran poder mágico que modifica tu apariencia.", FontTypeNames.FONTTYPE_FIGHT)
                        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(UserList(UserIndex).Char.CharIndex, 50, 0))
                        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(110, .Pos.X, .Pos.Y))
                    Else
                        Call WriteConsoleMsg(UserIndex, "Ya hás cambiado tu apariencia.", FontTypeNames.FONTTYPE_FIGHT)
                    End If
                End If
       End Select
       Call WriteUpdateUserStats(UserIndex)
    ...
    Case eOBJType.otMapaTesoro
        If .flags.Muerto = 1 Then
            Call WriteConsoleMsg(UserIndex, "¡¡Estas muerto!! No puedes visualizar el mapa del tesoro de esta forma!. ", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        
        Dim Pos As WorldPos
        Pos.map = 1
        Pos.X = RandomNumber(164, 257)
        Pos.Y = RandomNumber(16, 118)
        
     
        
        Call WriteConsoleMsg(UserIndex, "El Tesoro se encuentra en Mapa: " & Pos.map & " (" & Pos.X & "," & Pos.Y & "). Si no te das prisa lo encontrarán!", FontTypeNames.FONTTYPE_INFOBOLD)
      
        Dim CofreTesoro As Obj
        CofreTesoro.ObjIndex = 11
        CofreTesoro.Amount = 1
        
        Call TirarItemAlPiso(Pos, CofreTesoro)
    ...
    Case eOBJType.otContenedores
        If .flags.Muerto = 1 Then
            Call WriteConsoleMsg(UserIndex, "¡¡Estas muerto!! No puedes abrir el cofre!!", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
            
      
        Dim Tesoro As Obj
        Tesoro.ObjIndex = 12 'ORO
        
        Dim i As Integer
        For i = 1 To 10
             Tesoro.Amount = RandomNumber(1, 10000)
            Call TirarItemAlPiso(.Pos, Tesoro)
        Next i
        'Quitamos del inv el item
        Call QuitarUserInvItem(UserIndex, Slot, 1)
        ...
        Dim CofreTesoroAbierto As Obj
        CofreTesoroAbierto.ObjIndex = 10
        CofreTesoroAbierto.Amount = 1
        
         If Not MeterItemEnInventario(UserIndex, CofreTesoroAbierto) Then
            Call TirarItemAlPiso(.Pos, MiObj)
        End If
        
    Case eOBJType.otContenedoresAbiertos
        If .flags.Muerto = 1 Then
            Call WriteConsoleMsg(UserIndex, "¡¡Estas muerto!! No puedes usar este cofre!!", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
            ...
            If TargObj.Cerrada = 1 Then
                  ...
                  If TargObj.Llave > 0 Then
                     If TargObj.clave = Obj.clave Then
                        ...
                        MapData(.flags.TargetObjMap).Tile(.flags.TargetObjX, .flags.TargetObjY).ObjInfo.ObjIndex _
                        = ObjData(MapData(.flags.TargetObjMap).Tile(.flags.TargetObjX, .flags.TargetObjY).ObjInfo.ObjIndex).IndexCerrada
                        .flags.TargetObj = MapData(.flags.TargetObjMap).Tile(.flags.TargetObjX, .flags.TargetObjY).ObjInfo.ObjIndex
                  ...
                  Else
                     If TargObj.clave = Obj.clave Then
                        MapData(.flags.TargetObjMap).Tile(.flags.TargetObjX, .flags.TargetObjY).ObjInfo.ObjIndex _
                        = ObjData(MapData(.flags.TargetObjMap).Tile(.flags.TargetObjX, .flags.TargetObjY).ObjInfo.ObjIndex).IndexCerradaLlave
    ...
    Case eOBJType.otPergaminos
        ...
        If Hechizos(ObjData(.Invent.Object(Slot).ObjIndex).HechizoIndex).ExclusivoClase <> "" Then
            If UCase$(Hechizos(ObjData(UserList(UserIndex).Invent.Object(Slot).ObjIndex).HechizoIndex).ExclusivoClase) <> UCase$(UserList(UserIndex).clase) Then
    ...
    Case eOBJType.otBarcos
        ...
        If .flags.NpcMonturaNumero > 0 Then
            Call WriteConsoleMsg(UserIndex, "No puedes dejar a la criatura que hás domado, libérala!.", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
...
Sub TirarTodosLosItems(ByVal UserIndex As Integer)
    ...
    If UserList(UserIndex).Invent.AnilloEqpObjIndex = OBJ_CARRO_MINERIA Then
        Porcent = 40
    Else
        Porcent = 100
    End If
    ...
    For i = 1 To MAX_INVENTORY_SLOTS
        ...
        If ItemIndex > 0 Then
             If ItemSeCae(ItemIndex) Then
                ...
                If ItemIndex = iMinerales.HierroCrudo Or _
                   ItemIndex = iMinerales.PlataCruda Or _
                   ItemIndex = iMinerales.OroCrudo Then
                    MiObj.Amount = Porcentaje(UserList(UserIndex).Invent.Object(i).Amount, Porcent)
                Else
...
For i = 1 To MAX_INVENTORY_SLOTS
    ...
    If ItemIndex > 0 Then
        If ItemSeCae(ItemIndex) And Not ItemNewbie(ItemIndex) Then
            ...
            Tilelibre UserList(UserIndex).Pos, NuevaPos, MiObj, True, True
            If NuevaPos.X <> 0 And NuevaPos.Y <> 0 Then
                If MapData(NuevaPos.map).Tile(NuevaPos.X, NuevaPos.Y).ObjInfo.ObjIndex = 0 Then Call DropObj(UserIndex, i, MAX_INVENTORY_OBJS, NuevaPos.map, NuevaPos.X, NuevaPos.Y)
            End If
        End If
    End If
Next i
...
Public Function UserNoPuedeUsarItem(ByVal UserIndex As Integer, ByVal ObjIndex As Integer) As Byte

    If ObjIndex = 0 Then
        UserNoPuedeUsarItem = 0
        Exit Function
    End If

    If ObjData(ObjIndex).OBJType = otArmadura Then
        If ClasePuedeUsarItem(UserIndex, ObjIndex) And _
            ...
            UserNoPuedeUsarItem = 0
        Else
            UserNoPuedeUsarItem = 1
            Exit Function
        End If
    ElseIf ClasePuedeUsarItem(UserIndex, ObjIndex) And _
        SexoPuedeUsarItem(UserIndex, ObjIndex) And _
        FaccionPuedeUsarItem(UserIndex, ObjIndex) Then
            UserNoPuedeUsarItem = 0
        UserNoPuedeUsarItem = 0
