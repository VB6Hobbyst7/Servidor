...
'*************************************************
    ...
    If Cant > 0 Then
        ...
        If CByte(Datos("EjercitoReal")) = 1 Then
            ...
            Call WriteConsoleMsg(sendIndex, "Ingresó en Nivel: " & CInt(Datos("NivelIngreso")) & " con " & CInt(Datos("MatadosIngreso")) & " Ciudadanos matados.", FontTypeNames.FONTTYPE_INFO)
            Call WriteConsoleMsg(sendIndex, "Veces que Ingresó: " & CByte(Datos("Reenlistadas")), FontTypeNames.FONTTYPE_INFO)
        ...
        ElseIf CByte(Datos("EjercitoCaos")) = 1 Then
            ...
            Call WriteConsoleMsg(sendIndex, "Ingresó en Nivel: " & Datos("NivelIngreso"), FontTypeNames.FONTTYPE_INFO)
            Call WriteConsoleMsg(sendIndex, "Veces que Ingresó: " & Datos("Reenlistadas"), FontTypeNames.FONTTYPE_INFO)
...
'**********************************************
    ...
    Else
        ...
        ElseIf Npclist(NpcIndex).Stats.Alineacion = 0 Then
                If Not Npclist(NpcIndex).MaestroUser > 0 And Npclist(NpcIndex).EquitandoBody = 0 Then   'mascotas nooo!
                    Call VolverCriminal(UserIndex)
                End If
...
'************************************************
    ...
    With UserList(UserIndex)
        ...
        Call WritePicInRender(UserIndex, ePicRenderType.BloodDie)
        ...
        If UserList(UserIndex).flags.Equitando = True Then
            Call QuitarMontura(UserIndex, False)
        End If

        'CONTADORES

        '<<<< CONGELADO >>>>
        If .Counters.Congelado > 0 Then
            Call DescongelarUser(UserIndex)
        End If

        '<<<< NADANDO >>>>
        If .flags.Nadando = True Then
            Call Muere_Embarcacion(UserIndex, False)
        End If
...
Sub WarpUserChar(ByVal UserIndex As Integer, ByVal map As Integer, ByVal X As Integer, ByVal Y As Integer, ByVal FX As Boolean, Optional Teletransported As Boolean = False)
    ...
    With UserList(UserIndex)
        ...
        'Chiquito
        If .flags.Chiquito = True Then
            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChiquito(.Char.CharIndex, True))
        End If
        
        'Congelado
        If .Counters.Congelado > 0 Then
            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCongelado(.Char.CharIndex, True))
        End If
...
'***************************************************
    ...
    If UserList(UserIndex).flags.UserLogged And Not UserList(UserIndex).Counters.Saliendo Then
        ...
        If UserList(UserIndex).flags.Equitando = True Then Call QuitarMontura(UserIndex, False)
        Call LiberarMontura(UserIndex)
        If UserList(UserIndex).flags.Nadando = True Then Call Muere_Embarcacion(UserIndex, True)
        If UserList(UserIndex).Counters.Chiquito > 0 Then Call Chiquitolin(UserIndex, False)
        If UserList(UserIndex).Counters.Congelado > 0 Then Call DescongelarUser(UserIndex)
...
Public Sub SetOculto(ByVal UserIndex As Integer, ByVal UserCharIndex As Integer, ByVal invisible As Boolean)
    With UserList(UserIndex)
        Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageSetOculto(UserCharIndex, invisible))
    End With
End Sub
