...
If frmMain.Visible Then frmMain.txStatus.Caption = "Cargando frases para los trabajadores."
Dim NPCI As Integer
Dim Leer As New clsIniReader
'Leo el dat de npc inteligentes
Call Leer.Initialize(DatPath & "FrasesTrabajadores.dat")
NumeroFraseTrabajador = 0
NumeroFraseTrabajador = val(Leer.GetValue("INIT", "NumeroFrases"))
'Si al menos hay un npc trabajador, sigo
If NumeroFraseTrabajador > 0 Then
    'En  base al numero de npcs, creo un array del tipo tNpcInteligente
    ReDim FraseTrabajadores(1 To NumeroFraseTrabajador) As tFraseTrabajador
    'Preparo el cargando
    frmCargando.cargar.min = 0
    frmCargando.cargar.max = NumeroFraseTrabajador
...
errhandler:
        MsgBox "Error cargando las frases para los trabajadores " & Err.Number & ": " & Err.Description
End Sub
...
On Error GoTo errhandler
'Informo en la consola que estoy cargando los npc
If frmMain.Visible Then frmMain.txStatus.Caption = "Cargando Rutas para los taladores."
Dim NPCI As Integer
Dim Leer As New clsIniReader
'Leo el dat de npc inteligentes
Call Leer.Initialize(DatPath & "RutaTaladores.dat")
NumeroRutaTalador = 0
NumeroRutaTalador = val(Leer.GetValue("INIT", "NumeroRutaTalador"))
'Si al menos hay un npc trabajador, sigo
If NumeroRutaTalador > 0 Then
    'En  base al numero de npcs, creo un array del tipo tNpcInteligente
    ReDim RutaTaladores(1 To NumeroRutaTalador) As tRutaTrabajador
    'Preparo el cargando
    frmCargando.cargar.min = 0
    frmCargando.cargar.max = NumeroRutaTalador
    frmCargando.cargar.value = 0
    'Leo npc por npc y lo cargo en el array
        For NPCI = 1 To NumeroRutaTalador
            RutaTaladores(NPCI).Ruta = Leer.GetValue("RUTATALADOR" & NPCI, "Ruta")
            RutaTaladores(NPCI).DireccionInicio = Leer.GetValue("RUTATALADOR" & NPCI, "DireccionInicio")
            RutaTaladores(NPCI).DireccionFin = Leer.GetValue("RUTATALADOR" & NPCI, "DireccionFin")
            'Avisa que un nuevo npc fue cargado
            frmCargando.cargar.value = frmCargando.cargar.value + 1
        Next NPCI
    End If
...
errhandler:
        MsgBox "Error cargando Rutas de los taladores " & Err.Number & ": " & Err.Description
End Sub
...
On Error GoTo errhandler
'Informo en la consola que estoy cargando los npc
If frmMain.Visible Then frmMain.txStatus.Caption = "Cargando Rutas para los herreros."
Dim NPCI As Integer
Dim Leer As New clsIniReader
'Leo el dat de npc inteligentes
Call Leer.Initialize(DatPath & "RutaHerreros.dat")
NumeroRutaHerrero = 0
NumeroRutaHerrero = val(Leer.GetValue("INIT", "NumeroRutaHerrero"))
'Si al menos hay un npc trabajador, sigo
If NumeroRutaHerrero > 0 Then
    'En  base al numero de npcs, creo un array del tipo tNpcInteligente
    ReDim RutaHerreros(1 To NumeroRutaHerrero) As tRutaTrabajador
    'Preparo el cargando
    frmCargando.cargar.min = 0
    frmCargando.cargar.max = NumeroRutaHerrero
    frmCargando.cargar.value = 0
    'Leo npc por npc y lo cargo en el array
        For NPCI = 1 To NumeroRutaHerrero
            RutaHerreros(NPCI).Ruta = Leer.GetValue("RUTAHERRERO" & NPCI, "Ruta")
            RutaHerreros(NPCI).DireccionInicio = Leer.GetValue("RUTAHERRERO" & NPCI, "DireccionInicio")
            RutaHerreros(NPCI).DireccionFin = Leer.GetValue("RUTAHERRERO" & NPCI, "DireccionFin")
            'Avisa que un nuevo npc fue cargado
            frmCargando.cargar.value = frmCargando.cargar.value + 1
        Next NPCI
    End If
...
errhandler:
        MsgBox "Error cargando Rutas de los herreros " & Err.Number & ": " & Err.Description
End Sub
...
On Error GoTo errhandler
'Informo en la consola que estoy cargando los npc
If frmMain.Visible Then frmMain.txStatus.Caption = "Cargando Rutas para los carpinteros."
Dim NPCI As Integer
Dim Leer As New clsIniReader
'Leo el dat de npc inteligentes
Call Leer.Initialize(DatPath & "RutaCarpinteros.dat")
NumeroRutaCarpintero = 0
NumeroRutaCarpintero = val(Leer.GetValue("INIT", "NumeroRutaCarpintero"))
'Si al menos hay un npc trabajador, sigo
If NumeroRutaCarpintero > 0 Then
    'En  base al numero de npcs, creo un array del tipo tNpcInteligente
    ReDim RutaCarpinteros(1 To NumeroRutaCarpintero) As tRutaTrabajador
    'Preparo el cargando
    frmCargando.cargar.min = 0
    frmCargando.cargar.max = NumeroRutaCarpintero
    frmCargando.cargar.value = 0
    'Leo npc por npc y lo cargo en el array
        For NPCI = 1 To NumeroRutaCarpintero
            RutaCarpinteros(NPCI).Ruta = Leer.GetValue("RUTACARPINTERO" & NPCI, "Ruta")
            RutaCarpinteros(NPCI).DireccionInicio = Leer.GetValue("RUTACARPINTERO" & NPCI, "DireccionInicio")
            RutaCarpinteros(NPCI).DireccionFin = Leer.GetValue("RUTACARPINTERO" & NPCI, "DireccionFin")
            'Avisa que un nuevo npc fue cargado
            frmCargando.cargar.value = frmCargando.cargar.value + 1
        Next NPCI
    End If
...
errhandler:
        MsgBox "Error cargando Rutas de los carpinteros " & Err.Number & ": " & Err.Description
End Sub
...
Dim Leer As New clsIniReader
'Leo el dat de npc inteligentes
Call Leer.Initialize(DatPath & "RutaMineros.dat")
NumeroRutaMinero = 0
NumeroRutaMinero = val(Leer.GetValue("INIT", "NumeroRutaMinero"))
'Si al menos hay un npc trabajador, sigo
If NumeroRutaMinero > 0 Then
    'En  base al numero de npcs, creo un array del tipo tNpcInteligente
    ReDim RutaMineros(1 To NumeroRutaMinero) As tRutaTrabajador
    'Preparo el cargando
    frmCargando.cargar.min = 0
    frmCargando.cargar.max = NumeroRutaMinero
    frmCargando.cargar.value = 0
    'Leo npc por npc y lo cargo en el array
        For NPCI = 1 To NumeroRutaMinero
            RutaMineros(NPCI).Ruta = Leer.GetValue("RUTAMINERO" & NPCI, "Ruta")
            RutaMineros(NPCI).DireccionInicio = Leer.GetValue("RUTAMINERO" & NPCI, "DireccionInicio")
            RutaMineros(NPCI).DireccionFin = Leer.GetValue("RUTAMINERO" & NPCI, "DireccionFin")
            'Avisa que un nuevo npc fue cargado
            frmCargando.cargar.value = frmCargando.cargar.value + 1
        Next NPCI
    End If
...
errhandler:
        MsgBox "Error cargando Rutas de los mineros " & Err.Number & ": " & Err.Description
End Sub
...
On Error GoTo errhandler
'Informo en la consola que estoy cargando los npc
If frmMain.Visible Then frmMain.txStatus.Caption = "Cargando Rutas para los trabajadores."
Dim NPCI As Integer
Dim Leer As New clsIniReader
'Leo el dat de npc inteligentes
Call Leer.Initialize(DatPath & "RutaPescadores.dat")
NumeroRutaPescador = 0
NumeroRutaPescador = val(Leer.GetValue("INIT", "NumeroRutaPescador"))
'Si al menos hay un npc trabajador, sigo
If NumeroRutaPescador > 0 Then
    'En  base al numero de npcs, creo un array del tipo tNpcInteligente
    ReDim RutaPescadores(1 To NumeroRutaPescador) As tRutaTrabajador
    'Preparo el cargando
    frmCargando.cargar.min = 0
    frmCargando.cargar.max = NumeroRutaPescador
    frmCargando.cargar.value = 0
    'Leo npc por npc y lo cargo en el array
        For NPCI = 1 To NumeroRutaPescador
            RutaPescadores(NPCI).Ruta = Leer.GetValue("RUTAPESCADOR" & NPCI, "Ruta")
            RutaPescadores(NPCI).DireccionInicio = Leer.GetValue("RUTAPESCADOR" & NPCI, "DireccionInicio")
            RutaPescadores(NPCI).DireccionFin = Leer.GetValue("RUTAPESCADOR" & NPCI, "DireccionFin")
            'Avisa que un nuevo npc fue cargado
            frmCargando.cargar.value = frmCargando.cargar.value + 1
        Next NPCI
    End If
...
errhandler:
        MsgBox "Error cargando Rutas de los pescadores " & Err.Number & ": " & Err.Description
End Sub
...
On Error GoTo errhandler
'Informo en la consola que estoy cargando los npc
If frmMain.Visible Then frmMain.txStatus.Caption = "Cargando configuraciòn de trabajadores."
Dim NPCI As Integer
Dim Leer As New clsIniReader
'Leo el dat de npc inteligentes
Call Leer.Initialize(DatPath & "ConfigurarTrabajadores.dat")
'Se carga la configuracion en variables globales
TiempoMinimoTrabajo = val(Leer.GetValue("TIEMPO_ESPERA", "TiempoMinimoTrabajo"))
TiempoMaximoTrabajo = val(Leer.GetValue("TIEMPO_ESPERA", "TiempoMaximoTrabajo"))
TiempoMinimoHablar = val(Leer.GetValue("TIEMPO_ESPERA", "TiempoMinimoHablar"))
TiempoMaximoHablar = val(Leer.GetValue("TIEMPO_ESPERA", "TiempoMaximoHablar"))
TiempoMinimoLogin = val(Leer.GetValue("TIEMPO_ESPERA", "TiempoMinimoLogin"))
TiempoMaximoLogin = val(Leer.GetValue("TIEMPO_ESPERA", "TiempoMaximoLogin"))
TiempoMinimoResucitar = val(Leer.GetValue("TIEMPO_ESPERA", "TiempoMinimoResucitar"))
TiempoMaximoResucitar = val(Leer.GetValue("TIEMPO_ESPERA", "TiempoMaximoResucitar"))
TiempoMinimoComerBeber = val(Leer.GetValue("TIEMPO_ESPERA", "TiempoMinimoComerBeber"))
TiempoMaximoComerBeber = val(Leer.GetValue("TIEMPO_ESPERA", "TiempoMaximoComerBeber"))
IntervaloMinimoTrabajo = val(Leer.GetValue("INTERVALO", "IntervaloMinimoTrabajo"))
IntervaloMaximoTrabajo = val(Leer.GetValue("INTERVALO", "IntervaloMaximoTrabajo"))
CantidadMinimaBotManiana = val(Leer.GetValue("TURNOS", "CantidadMinimaBotManiana"))
CantidadMaximaBotManiana = val(Leer.GetValue("TURNOS", "CantidadMaximaBotManiana"))
CantidadMinimaBotTarde = val(Leer.GetValue("TURNOS", "CantidadMinimaBotTarde"))
CantidadMaximaBotTarde = val(Leer.GetValue("TURNOS", "CantidadMaximaBotTarde"))
CantidadMinimaBotNoche = val(Leer.GetValue("TURNOS", "CantidadMinimaBotNoche"))
CantidadMaximaBotNoche = val(Leer.GetValue("TURNOS", "CantidadMaximaBotNoche"))
CantidadMinimaBotMadrugada = val(Leer.GetValue("TURNOS", "CantidadMinimaBotMadrugada"))
CantidadMaximaBotMadrugada = val(Leer.GetValue("TURNOS", "CantidadMaximaBotMadrugada"))

Set Leer = Nothing
    Exit Sub

errhandler:
        MsgBox "Error cargando la conf. de los trabajadores " & Err.Number & ": " & Err.Description
End Sub
...
On Error GoTo errhandler
'Informo en la consola que estoy cargando los npc
If frmMain.Visible Then frmMain.txStatus.Caption = "Cargando NPCs Trabajadores."
Dim NPCI As Integer
Dim Leer As New clsIniReader
'Leo el dat de npc inteligentes
Call Leer.Initialize(DatPath & "Trabajadores.dat")
NumeroNpcTrabajador = 0
NumeroNpcTrabajador = val(Leer.GetValue("INIT", "NumeroTrabajadores"))
'Si al menos hay un npc trabajador, sigo
    If NumeroNpcTrabajador > 0 Then
        'En  base al numero de npcs, creo un array del tipo tNpcInteligente
        ReDim NpcTrabajadores(1 To NumeroNpcTrabajador) As tNpcTrabajador
        'Preparo el cargando
        frmCargando.cargar.min = 0
        frmCargando.cargar.max = NumeroNpcTrabajador
        frmCargando.cargar.value = 0
        'Leo npc por npc y lo cargo en el array
        For NPCI = 1 To NumeroNpcTrabajador
            NpcTrabajadores(NPCI).NumeroNPC = val(Leer.GetValue("TRABAJADOR" & NPCI, "NumeroNPC"))
            NpcTrabajadores(NPCI).Profesion = Leer.GetValue("TRABAJADOR" & NPCI, "Profesion")
            'Avisa que un nuevo npc fue cargado
            frmCargando.cargar.value = frmCargando.cargar.value + 1
        Next NPCI
    End If
...
errhandler:
        MsgBox "Error cargando Trabajadores.dat " & Err.Number & ": " & Err.Description
...
Dim Leer As New clsIniReader
'Leo el dat de npc inteligentes
Call Leer.Initialize(DatPath & "NpcInteligente.dat")
NumeroNpcInteligente = 0
NumeroNpcInteligente = val(Leer.GetValue("INIT", "NumeroNpcInteligentes"))
'En  base al numero de npcs, creo un array del tipo tNpcInteligente
ReDim NpcInteligentes(1 To NumeroNpcInteligente) As tNpcInteligente
'Preparo el cargando
frmCargando.cargar.min = 0
frmCargando.cargar.max = NumeroNpcInteligente
frmCargando.cargar.value = 0
...
For NPCI = 1 To NumeroNpcInteligente
    ...
    NpcInteligentes(NPCI).Fijo = val(Leer.GetValue("NpcInteligente" & NPCI, "Fijo"))
    NpcInteligentes(NPCI).TiempoEsperaMinuto = val(Leer.GetValue("NpcInteligente" & NPCI, "TiempoEsperaMinuto"))
    NpcInteligentes(NPCI).CrearInicioServer = val(Leer.GetValue("NpcInteligente" & NPCI, "CrearInicioServer"))
    NpcInteligentes(NPCI).TiempoRenacer = val(Leer.GetValue("NpcInteligente" & NPCI, "TiempoRenacer"))
    NpcInteligentes(NPCI).Mana = val(Leer.GetValue("NpcInteligente" & NPCI, "Mana"))
    NpcInteligentes(NPCI).PocionesRojas = val(Leer.GetValue("NpcInteligente" & NPCI, "PocionesRojas"))
    NpcInteligentes(NPCI).PocionesAzules = val(Leer.GetValue("NpcInteligente" & NPCI, "PocionesAzules"))
    NpcInteligentes(NPCI).OroInicial = val(Leer.GetValue("NpcInteligente" & NPCI, "OroInicial"))
    NpcInteligentes(NPCI).IncrementoOro = val(Leer.GetValue("NpcInteligente" & NPCI, "IncrementoOro"))
    NpcInteligentes(NPCI).OroMaximo = val(Leer.GetValue("NpcInteligente" & NPCI, "OroMaximo"))
    NpcInteligentes(NPCI).TiraRemo = val(Leer.GetValue("NpcInteligente" & NPCI, "TiraRemo"))
    NpcInteligentes(NPCI).TiraInmo = val(Leer.GetValue("NpcInteligente" & NPCI, "TiraInmo"))
    NpcInteligentes(NPCI).TiraApoca = val(Leer.GetValue("NpcInteligente" & NPCI, "TiraApoca"))
    NpcInteligentes(NPCI).TiraDescarga = val(Leer.GetValue("NpcInteligente" & NPCI, "TiraDescarga"))
    NpcInteligentes(NPCI).TiraTormentaFuego = val(Leer.GetValue("NpcInteligente" & NPCI, "TiraTormentaFuego"))
    NpcInteligentes(NPCI).TiraVerInvi = val(Leer.GetValue("NpcInteligente" & NPCI, "TiraVerInvi"))
    NpcInteligentes(NPCI).NroHechizoInvocacion = val(Leer.GetValue("NpcInteligente" & NPCI, "NroHechizoInvocacion"))
    NpcInteligentes(NPCI).MsjAgresorInvi = Leer.GetValue("NpcInteligente" & NPCI, "MsjAgresorInvi")
    NpcInteligentes(NPCI).MsjNuevoAgresor = Leer.GetValue("NpcInteligente" & NPCI, "MsjNuevoAgresor")
    NpcInteligentes(NPCI).MsjAgresorRandom1 = Leer.GetValue("NpcInteligente" & NPCI, "MsjAgresorRandom1")
    NpcInteligentes(NPCI).MsjAgresorRandom2 = Leer.GetValue("NpcInteligente" & NPCI, "MsjAgresorRandom2")
    NpcInteligentes(NPCI).MsjAgresorRandom3 = Leer.GetValue("NpcInteligente" & NPCI, "MsjAgresorRandom3")
    NpcInteligentes(NPCI).MsjAvisoPosibleAgresor = Leer.GetValue("NpcInteligente" & NPCI, "MsjAvisoPosibleAgresor")
    NpcInteligentes(NPCI).MsjNoMolestes = Leer.GetValue("NpcInteligente" & NPCI, "MsjNoMolestes")
    NpcInteligentes(NPCI).MsjAlHacerClick = Leer.GetValue("NpcInteligente" & NPCI, "MsjAlHacerClick")
    'Se cargan las rutas
    NpcInteligentes(NPCI).NroRutas = val(Leer.GetValue("NpcInteligente" & NPCI, "NRORUTAS"))
    ReDim RutaNpcInteligente(1 To NpcInteligentes(NPCI).NroRutas) As tRutaNpcInteligente
    
    Dim indiceRuta As Integer
    Dim rutaNpc As tRutaNpcInteligente
    For indiceRuta = 1 To NpcInteligentes(NPCI).NroRutas
         rutaNpc.Camino = Leer.GetValue("NpcInteligente" & NPCI, "Ruta" & indiceRuta)
         RutaNpcInteligente(indiceRuta) = rutaNpc
    Next indiceRuta
    
    NpcInteligentes(NPCI).Ruta = RutaNpcInteligente
...
errhandler:
 MsgBox "Error cargando NpcInteligente.dat " & Err.Number & ": " & Err.Description
